...
class HighestRewardPerDistanceBotBaseLogic(BaseLogic):
    def __init__(self):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        for diamond in diamonds:
            ...
            distance_from_base = self.get_distance(diamond[0], board_bot.properties.base)
            benefit = self.calculate_benefit(diamond[1], distance_from_bot + distance_from_base)
            temp.append((diamond[0], diamond[1], distance_from_bot, benefit))
        sorted_temp = sorted(temp, key=lambda x: x[3], reverse=True)
