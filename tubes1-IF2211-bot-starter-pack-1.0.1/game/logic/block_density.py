...
class BlockDensityLogic(BaseLogic):
    ...
    def get_distance(self, current_position: Position, target_position: Position):
        return abs(current_position.x - target_position.x) + abs(current_position.y - target_position.y)

    def calculate_benefit(self, point, distance) :
        if distance == 0:
            return 0
        else :
            return (point / distance) * 100000
        
    def is_teleporting_closer(self, bot_position, target_position, teleporter_pairs):
    ...
    def get_direction_yfirst(self, current_x, current_y, dest_x, dest_y):
